      ...
      >
        <div className='flex items-start justify-between gap-3'>
          <div className='space-y-1'>
            <h3 className='text-sm font-semibold text-gray-900'>{product.name || 'Без названия'}</h3>
        ...
        {!product.active && (
          <div className='mt-3 inline-flex rounded-full bg-gray-200/80 px-2 py-1 text-xs text-gray-600'>Не активен</div>
        )}
        {outOfStock && (
          <div className='mt-2 inline-flex rounded-full bg-amber-100/80 px-2 py-1 text-xs text-amber-700'>Нет в наличии</div>
        )}
      </div>
    </label>
  );
